...
- Whenever the user asks Claude to remember, update, or correct a normal memory, perform Claude's memory update and then call sync_claude_memory with the complete updated snapshot in the same turn. If a complete snapshot is unavailable, call save_memory with the exact user-approved fact instead. Never send incognito content, unsupported inferences, hidden memory, or ordinary conversation-only details.
...
- Call save_memory only after the user explicitly approves the exact durable fact being saved. A direct request to Claude to remember that same fact counts as approval to mirror it to Mora.
